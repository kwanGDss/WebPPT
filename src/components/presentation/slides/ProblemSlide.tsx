    ...
    <Slide>
      <div className="max-w-7xl w-full px-6 z-10">
        <div className="text-center mb-8">
          ...
          <h2 className="text-5xl font-bold text-gray-900 mb-4">현재 연애 시장의 문제점</h2>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          {PROBLEM_ISSUES.map((item, index) => (
            <div key={index} className="backdrop-blur-xl bg-white/70 border border-white/50 rounded-2xl shadow-xl p-8 group hover:bg-white/20 transition-all duration-300 h-full flex flex-col">
              ...
              <p className="text-gray-600 mb-6 leading-relaxed flex-grow text-lg">{item.problem}</p>
              <div className="mt-auto space-y-3">
                <div className="bg-red-50 border border-red-200 rounded-lg p-3">
                  <p className="text-red-800 font-medium text-base">{item.stat}</p>
                </div>
                <div className="flex items-center gap-2 text-red-500">
                  <div className="w-2 h-2 bg-red-500 rounded-full" />
                  <span className="text-base font-medium">{item.impact}</span>
                </div>
              </div>
            </div>
          ))}
